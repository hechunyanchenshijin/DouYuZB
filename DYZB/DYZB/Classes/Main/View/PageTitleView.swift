...
extension PageTitleView {
    ...
    private func setupTitleLabels() {
        ...
        for (index, title) in titles.enumerated() {
            ...
            let tapCes = UITapGestureRecognizer(target: self, action: #selector(self.titleLabelClick(tapGes:)))
            label.addGestureRecognizer(tapCes)
...
extension PageTitleView {
    @objc fileprivate func titleLabelClick(tapGes : UITapGestureRecognizer) {
        ...
        // 6. 通知代理
        delegate?.PageTitleViewD(titleView: self, selectedIndex: currentIndex)
    }
}
